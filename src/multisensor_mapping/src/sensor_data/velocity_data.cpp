...
void VelocityData::TransformCoordinate(Eigen::Matrix4f transform_matrix) {
    // transform linear and angular velocity from imu frame to lidar frame
    Eigen::Matrix4d matrix = transform_matrix.cast<double>();
    
    // get transformation T_imu_lidar
    ...
    // add velocity component generated by rotation
    ...
    // transform velocities in IMU frame to lidar frame
    w = R.transpose() * w; // w_lidar = T_lidar_imu * w_imu
    v = R.transpose() * v; // v_lidar = T_lidar_imu * v_imu
...
// transform linear and angular velocity from ned to enu
