#include "multisensor_mapping/mapping/loop_closing/loop_closing.hpp"
...
bool LoopClosing::InitWithConfig() {
    ...
    std::cout << "-----------------Loop-Closing Detection Init-------------------" << std::endl;
    ...
    // initialize loop closure method
...
bool LoopClosing::Update(
    ...
    // get scan context information from current key frame
...
bool LoopClosing::DetectNearestKeyFrame(
    ...
    // this is needed for valid local map
...
bool LoopClosing::CloudRegistration(
    ...
    // load a submap for loop closure registration
    ...
    // compute relative pose: T_loop_current = T_loop_world * T_world_current
...
bool LoopClosing::JointMap(
    ...
    // initialize map pose as loop closure pose
    ...
    for (int i = key_frame_index - extend_frame_num_; i < key_frame_index + extend_frame_num_; ++i) {
        // load back surrounding key scans
        ...
        // transform surrounding key scans to map frame
