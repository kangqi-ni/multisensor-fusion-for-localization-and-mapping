...
bool LIOBackEnd::InitWithConfig() {
    ...
    std::cout << "-----------------LIO Backend Init-------------------" << std::endl;

    // init data storage paths
...
bool LIOBackEnd::InitGraphOptimizer(const YAML::Node& config_node) {
    ...
    std::cout << "\tOptimizer:" << graph_optimizer_type << '\n' << std::endl;
...
bool LIOBackEnd::UpdateIMUPreIntegration(const IMUData &imu_data) {
    if (!graph_optimizer_config_.use_imu_pre_integration || imu_pre_integrator_ptr_ == nullptr)
        return false;
...
bool LIOBackEnd::UpdateOdoPreIntegration(const VelocityData &velocity_data) {
    if (!graph_optimizer_config_.use_odo_pre_integration || odo_pre_integrator_ptr_ == nullptr)
        return false;
...
bool LIOBackEnd::CheckNewKeyFrame(
    ...
    if (fabs(laser_odom.pose(0,3) - last_laser_pose.pose(0,3)) + 
        ...
        // for IMU pre-integration debugging
    ...
    if (has_new_key_frame_) {
        ...
        // create key frame index for lidar scan, relative pose measurement
        current_key_frame_.pose = laser_odom.pose;

        // create key frame index for GNSS measurement, full LIO state
...
bool LIOBackEnd::AddNodeAndEdge(const PoseData& gnss_data) {
    ...
    if (N > 1) {
        // get relative pose measuremen T_last_current = T_last_world * T_world_current
