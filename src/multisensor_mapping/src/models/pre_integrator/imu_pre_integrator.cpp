...
IMUPreIntegrator::IMUPreIntegrator(const YAML::Node& node) {
    ...
    LOG(INFO) << "\nIMU Pre-Integration params:\n"
              << "\tgravity magnitude: " << EARTH.GRAVITY_MAGNITUDE << '\n'
              << '\n'
              << "\tprocess noise:\n"
              << "\t\tmeasurement:\n"
              << "\t\t\taccel.: " << COV.MEASUREMENT.ACCEL << '\n'
              << "\t\t\tgyro.: " << COV.MEASUREMENT.GYRO << '\n'
              << "\t\trandom_walk:\n"
              << "\t\t\taccel.: " << COV.RANDOM_WALK.ACCEL << '\n'
              << "\t\t\tgyro.: " << COV.RANDOM_WALK.GYRO << '\n'
              << std::endl;
...
bool IMUPreIntegrator::Update(const IMUData &imu_data) {
    // make sure there are imu data to preintegrate
