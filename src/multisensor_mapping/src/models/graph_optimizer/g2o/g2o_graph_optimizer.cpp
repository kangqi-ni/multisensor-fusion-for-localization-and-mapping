...
#include "multisensor_mapping/tools/tic_toc.hpp"
...
bool G2oGraphOptimizer::Optimize() {
    ...
    LOG(INFO) << "\n------ Finish Iteration " << ++optimize_cnt << " of Backend Optimization -------" << std::endl
              << "\tNum. Vertices: " << graph_ptr_->vertices().size() << '\n' 
              << "\tNum. Edges: " << graph_ptr_->edges().size() << '\n'
              << "\tNum. Iterations: " << iterations << "/" << max_iterations_num_ << '\n'
              << "\tTime Consumption: " << optimize_time.toc() << '\n'
...
void G2oGraphOptimizer::ShowIMUPreIntegrationResidual(
    ...
    LOG(INFO) << "IMU Pre-Integration Measurement: " << '\n'
                << "\tT: " << T << '\n'
                << "\talpha:" << '\n'
                << "\t\t" << r_p.x() << ", " << r_p.y() << ", " << r_p.z() << '\n'
                << "\ttheta:" << '\n'
...
// NOTE: orientation prior seems to corrupt optimization accuracy so not added
