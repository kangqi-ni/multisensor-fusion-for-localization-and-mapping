#ifndef MULTISENSOR_LOCALIZATION_MODELS_SLIDING_WINDOW_FACTOR_PRVAG_MAP_MATCHING_POSE_HPP_
#define MULTISENSOR_LOCALIZATION_MODELS_SLIDING_WINDOW_FACTOR_PRVAG_MAP_MATCHING_POSE_HPP_
  ...
  virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const {
    ...
    if (jacobians) {
      if (jacobians[0]) {
        ...
        jacobians_eigen.block<3,3>(INDEX_R, INDEX_R) = JacobianRInv(jacobians_eigen.block<3,1>(INDEX_R, 0)) * ori_prior.matrix();
...
private:
  static Eigen::Matrix3d JacobianRInv(const Eigen::Vector3d &w) {
      ...
      if ( theta > 1e-5 ) {
          // // derivation in On-Manifold Preintegration for Real-Time Visual-Inertial Odometry
          ...
          // derivation in State Estimation for Robotics
...
#endif // MULTISENSOR_LOCALIZATION_MODELS_SLIDING_WINDOW_FACTOR_PRVAG_MAP_MATCHING_POSE_HPP_
