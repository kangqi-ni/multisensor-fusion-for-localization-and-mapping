...
bool CeresSlidingWindow::Optimize() {
    ...
    if ((kWindowSize + 1 <= N)) {
        ...
        // 2. map matching pose constraint
        if (!residual_blocks_.map_matching_pose.empty()) {
        ...
        // 4. IMU pre-integration constraint
        ...
        LOG(INFO) << "------ Finish Iteration " << ++optimization_count << " of Sliding Window Optimization -------\n"
