...
#include "multisensor_localization/models/cloud_filter/voxel_filter.hpp"
#include "multisensor_localization/models/cloud_filter/no_filter.hpp"

#include "multisensor_localization/models/registration/ndt_registration.hpp"
#include "multisensor_localization/models/registration/ndt_cpu/ndt_cpu_registration.hpp"
...
bool Matching::InitWithConfig() {
    ...
    // init lidar frontend for relative pose estimation
...
bool Matching::InitGlobalMap(const YAML::Node& config_node) {
    ...
    LOG(INFO) << "\tLoad Global Map (# points: " << global_map_ptr_->points.size() << ')';
...
bool Matching::SetGNSSPose(const Eigen::Matrix4f& gnss_pose) {
    ...
    if (gnss_cnt == 0) {
        // set local map at first gnss pose
...
bool Matching::Update(
    ...
    // compute step transformation T_last_current
